...
let num_of_chars chars is_float =
  if is_float
  then Float (float_of_string (chars_to_str chars))
...
let rec scan_string (ctx : ctx) (chars : char list) : (ctx * string, Error.t) result =
  ...
  | Some '\\' ->
    (match maybe_get ctx.source (ctx.idx + 1) with
     | Some c -> scan_string (advance ~n:2 ctx) (c :: chars)
     | _ -> scan_string (advance ctx) ('\\' :: chars))
  | Some char -> scan_string (advance ctx) (char :: chars)
  | None -> Error { line = ctx.line; column = ctx.column; msg = "unterminated string" }
;;
...
let rec scan_number (ctx : ctx) (chars : char list) (in_float : bool)
  ...
  | Some '.' -> Error { line = ctx.line; column = ctx.column; msg = "already in a float" }
...
let rec scan_loop (ctx : ctx) (tokens : Token.t list) : (Token.t list, Error.t) result =
  let make_tok (typ : token_type) (lexeme : string) : Token.t =
    { typ; lexeme; literal = None; line = ctx.line }
  in
  let make_err (ctx : ctx) (msg : string) : scan_res =
    Err { line = ctx.line; column = ctx.column; msg }
  in
  ...
  let lift_tok (tok : Token.t) : scan_res =
    let lexeme_len = String.length tok.lexeme in
    Tok { tok; ctx = advance ~n:lexeme_len ctx }
  in
  let lift_lit_tok (ctx' : ctx) (typ : token_type) (literal : lit option) : scan_res =
    Tok
  ...
  in
  match get_from_src ctx with
  | None -> Ok (List.rev (make_tok EOF "" :: tokens))
  | Some c ->
    let res : scan_res =
      match c with
      ...
      | '!' ->
        lift_tok (if match_next '=' then make_tok BANG_EQUAL "!=" else make_tok BANG "!")
      | '=' ->
        lift_tok
          (if match_next '=' then make_tok EQUAL_EQUAL "==" else make_tok EQUAL "=")
      | '<' ->
        lift_tok (if match_next '=' then make_tok LESS_EQUAL "<=" else make_tok LESS "<")
      | '>' ->
        lift_tok
          (if match_next '=' then make_tok GREATER_EQUAL ">=" else make_tok GREATER ">")
      (* maybe comments *)
      | '/' ->
        if match_next '/'
        then Skip (consume_comment ctx)
      ...
      (* otherwise *)
      | c -> make_err ctx ("unexpected character: " ^ to_str c)
    in
    (match res with
